create or replace package body dbops.dbops_lock_pkg is
    ...
    -- Check for existing locks
        ...
        close c_lock;
        if l_locked_rec.locked_by is not null then
            raise_application_error(-20001, 'Object is already locked by '
                                            || l_locked_rec.locked_by
                                            || ' since '
                                            || l_locked_rec.locked_on);
        end if;
    ...
    -- Check if object exists
        ...
        from
            dba_objects
        where
                object_type = upper(p_object_type)
            and object_name = upper(p_object_name)
            and owner = upper(p_owner);

        if l_found = 0 then
            raise_application_error(-20001, 'Object '
                                            || p_owner
                                            || '.'
                                            || p_object_name
                                            || ' of type '
    ...
    -- TODO: Implement administrator release option
        ...
        if l_locked_rec.locked_by != l_user then
            raise_application_error(-20001, 'Object is locked by '
                                            || l_locked_rec.locked_by
                                            || ' and cannot be released by '
                                            || l_user);
        end if;
  ...
  -- Print locks only works when serverout is on
    ...
    -- Print locks to dbms_output
        for l_rec in (
            ...
            dbms_output.put_line(l_rec.owner
                                 || '.'
                                 || l_rec.object_name
                                 || ' ('
                                 || l_rec.object_type
                                 ...
                                 || l_rec.locked_on
                                 || case
                when l_rec.notes is not null then
                    ', ' || l_rec.notes
                else null
            end);
        end loop;
...
-- sqlcl_snapshot {"hash":"ec839a82fb6a131426edf8bde5b73434ec7df022","type":"PACKAGE_BODY","name":"DBOPS_LOCK_PKG","schemaName":"DBOPS"}